...
# Diccionario global para representar las 68 casillas externas del tablero
casillas = {i: [] for i in range(1, 69)}

# Listas con posiciones especiales
casillas_seguras = [9, 17, 26, 34, 43, 51, 60, 68]
casillas_salida = [5, 22, 39, 56]
intentos_sin_salida = {}

# Variables globales para reglas de pares consecutivos y seguimiento de fichas
pares_consecutivos = 0
ultima_ficha_movida = None


# Esta función permite identificar si una casilla es segura o de salida
def es_seguro_o_salida(casilla):
    return casilla in casillas_seguras or casilla in casillas_salida


# Verifica si en el camino de una ficha hay un bloqueo por dos fichas de un mismo jugador
def hay_bloqueo_en_camino(jugador, posicion_actual, pasos):
    for i in range(1, pasos + 1):
        pos = (posicion_actual + i - 1) % 68 + 1
...
# Lógica de captura: si hay una ficha enemiga sola en la nueva casilla, se captura
def capturar_si_corresponde(ficha, pasos, jugadores):
    nueva_pos = (ficha.posicion + pasos - 1) % 68 + 1
...
# Se configuran los jugadores: nombre, color, casilla de salida y zona de llegada
...
# Controla la lógica del turno de cada jugador, incluyendo movimientos, pares y reglas especiales
def turno_jugador(jugador, jugadores, dado):
    ...
    # Regla: tres pares consecutivos castigan a la última ficha
...
# Esta es la función principal del juego. Controla el flujo por turnos y verifica repeticiones por pares.
def iniciar_juego():
    jugadores = configurar_jugadores()
    dado = Dado(modo='desarrollador')  # Se puede cambiar a 'real' para modo aleatorio
    turno = 0
